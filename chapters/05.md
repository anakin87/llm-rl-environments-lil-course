...
In the [previous chapter](04.md), we used our Tic Tac Toe environment to evaluate `openai/gpt-5-mini`, and `LiquidAI/LFM2-2.6B`.
...
In the [last chapter](04.md), we saw that `LiquidAI/LFM2-2.6B` follows format less than 30% of the time and provides
...
Assuming that the environment is installed (see the [last chapter](04.md) for instructions), we can generate 200 examples with opponents
of different skills:
```bash
...
![Generating synthetic data](../images/synthetic_data.gif)

The dataset is pushed to the Hugging Face Hub. You can inspect my generated data [here](https://huggingface.co/datasets/anakin87/tictactoe).
...
The filtered dataset is available [here](https://huggingface.co/datasets/anakin87/tictactoe-filtered) and contains 174 examples.
...
In this chapter, we structured our training and saw how easy it is to generate synthetic data using a Verifiers environment with a strong model. Now everything is ready to [start training](06.md)!

---

[⬅️ Previous: Evaluating existing models](04.md) | [Next: Supervised Fine-Tuning warm-up ➡️](06.md)