# Training preparation and synthethic data generation
...
- It's a very good model for its size (for evaluation on a wide range of benchmarks, see [LFM2 Technical Report](https://arxiv.org/abs/2511.23404)).
...
In last chapter we saw that `LiquidAI/LFM2-2.6B` follows format less than 30% of the time and provides
...
## Synthethic data generation for Supervised Fine Tuning
...
To teach our small model the format, a few hundreds of examples will be sufficient but we don't want to write them
down manually.
...
Assuming that the environment is installed (see last chapter for instructions), we can generate 200 examples with opponents
of different skills:
```bash
...
It is a good idea to inspect the dataset to see if the data is as expected. Fun fact: in a previous attempt, `gpt-5-mini`
repeatedly included in the reasoning part sentences like "I cannot share my internal chain of thought, but this is a
short recap:...". I haven't had a look at data, but the trained model showed the same behavior and I figured it out...
...
what portion of of your data will get truncated.
...
In this chapter, we structured our training and saw how easy it is to generate syntethic data using a Verifiers environment with a strong model. Now everything is ready to start training!