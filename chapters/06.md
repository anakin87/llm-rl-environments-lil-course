...
In the previous chapter, we saw that before applying Reinforcement Learning, it is a good idea to start
...
training. As we will see, it is quite easy to setup a quick SFT run.
...
I used a NVIDIA RTX Pro 6000 96GB, but you can use smaller GPUs, by lowering `batch_size`. Since this SFT warm-up requires just a few minutes, the GPU cost is negligible.
...
If you need Wandb support (for logging training runs) and Hugging Face support (for saving the model), run:
...
`seq_len` respects the token count we calculated in the last chapter and makes sure that most of our examples will not be truncated.
...
Final model is available [here](https://huggingface.co/anakin87/LFM2-2.6B-ttt-sft).
...
The training plots looks OK. If you are curious about more details, you can explore the "SFT" run in this [Wandb project](https://wandb.ai/stefanofiorucci/LFM2-2.6B%20Tic%20Tac%20Toe/table).

## Evaluation

We are now curious to see if SFT achieved the goal of improving the format of responses.
...
```bash
prime eval tictactoe -n 100 -r 1 -m anakin87/LFM2-2.6B-ttt-sft --api-base-url http://localhost:8000/v1 --api-key-var PLACEHOLDER_API_KEY -a '{"min_random_move_prob": 1.0, "max_random_move_prob": 1.0}'
```
...
```bash
prime eval tictactoe -n 100 -r 1 -m anakin87/LFM2-2.6B-ttt-sft --api-base-url http://localhost:8000/v1 --api-key-var PLACEHOLDER_API_KEY -a '{"min_random_move_prob": 0.0, "max_random_move_prob": 0.0}'
```
...
- **Games with invalid moved** dropped from >24% to ~14%
...
## Next up
There is still significant work to do to make our small model an excellent player, but that is a job for Reinforcement Learning. Go to the next chapter to see it in action!
